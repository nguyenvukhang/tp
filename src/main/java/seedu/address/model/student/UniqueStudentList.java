...
/**
 * A list of students that enforces uniqueness between its elements and does not
 * allow nulls. A student is considered unique by comparing using
 * {@code Student#isSameStudent(Student)}. As such, adding and updating of
 * students uses Student#isSameStudent(Student) for equality so as to ensure
 * that the student being added or updated is unique in terms of identity in the
 * UniqueStudentList. However, the removal of a student uses
 * Student#equals(Object) so as to ensure that the student with exactly the same
...
public class UniqueStudentList implements Iterable<Student> {
    ...
    private final ObservableList<Student> internalUnmodifiableList = FXCollections
                    .unmodifiableObservableList(internalList);

    /**
     * Returns true if the list contains an equivalent student as the given
     * argument.
     */
     ...
     * {@code target} must exist in the list. The student identity of
    ...
    /**
     * Removes the equivalent student from the list. The student must exist in the
     * list.
     */
