...
public class AddCommandParserTest {
    ...
    public void parse_allFieldsPresent_success() {
        Student expectedStudent = new StudentBuilder(BOB).withTags(VALID_TUTORIAL_1).build();
        ...
        Student expectedStudentMultipleTags = new StudentBuilder(BOB).withTags(VALID_TUTORIAL_1, VALID_TUTORIAL_2)
                        .build();
        assertParseSuccess(parser, NAME_DESC_BOB + PHONE_DESC_BOB + EMAIL_DESC_BOB + HANDLE_DESC_BOB + TUTORIAL_DESC_2
                        + TUTORIAL_DESC_1, new AddCommand(expectedStudentMultipleTags));
    }

    @Test
    public void parse_repeatedNonTagValue_failure() {
        String validExpectedStudentString = NAME_DESC_BOB + PHONE_DESC_BOB + EMAIL_DESC_BOB + HANDLE_DESC_BOB
                        + TUTORIAL_DESC_1;
        ...
        assertParseFailure(parser,
                        validExpectedStudentString + PHONE_DESC_AMY + EMAIL_DESC_AMY + NAME_DESC_AMY + HANDLE_DESC_AMY
                                        + validExpectedStudentString,
                        Messages.getErrorMessageForDuplicatePrefixes(PREFIX_NAME, PREFIX_HANDLE, PREFIX_EMAIL,
                                        PREFIX_PHONE));
    ...
    public void parse_optionalFieldsMissing_success() {
        ...
        assertParseSuccess(parser, NAME_DESC_AMY + PHONE_DESC_AMY + EMAIL_DESC_AMY + HANDLE_DESC_AMY,
    ...
    public void parse_compulsoryFieldMissing_failure() {
        ...
        // missing name prefix
        assertParseFailure(parser, VALID_NAME_BOB + PHONE_DESC_BOB + EMAIL_DESC_BOB + HANDLE_DESC_BOB, expectedMessage);

        // missing phone prefix
        assertParseFailure(parser, NAME_DESC_BOB + VALID_PHONE_BOB + EMAIL_DESC_BOB + HANDLE_DESC_BOB, expectedMessage);

        // missing email prefix
        assertParseFailure(parser, NAME_DESC_BOB + PHONE_DESC_BOB + VALID_EMAIL_BOB + HANDLE_DESC_BOB, expectedMessage);

        // missing address prefix
        assertParseFailure(parser, NAME_DESC_BOB + PHONE_DESC_BOB + EMAIL_DESC_BOB + VALID_HANDLE_BOB, expectedMessage);

        // all prefixes missing
        assertParseFailure(parser, VALID_NAME_BOB + VALID_PHONE_BOB + VALID_EMAIL_BOB + VALID_HANDLE_BOB,
                        expectedMessage);
    }
    ...
    public void parse_invalidValue_failure() {
        // invalid name
        assertParseFailure(parser, INVALID_NAME_DESC + PHONE_DESC_BOB + EMAIL_DESC_BOB + HANDLE_DESC_BOB
                        + TUTORIAL_DESC_2 + TUTORIAL_DESC_1, Name.MESSAGE_CONSTRAINTS);

        // invalid phone
        assertParseFailure(parser, NAME_DESC_BOB + INVALID_PHONE_DESC + EMAIL_DESC_BOB + HANDLE_DESC_BOB
                        + TUTORIAL_DESC_2 + TUTORIAL_DESC_1, Phone.MESSAGE_CONSTRAINTS);

        // invalid email
        assertParseFailure(parser, NAME_DESC_BOB + PHONE_DESC_BOB + INVALID_EMAIL_DESC + HANDLE_DESC_BOB
                        + TUTORIAL_DESC_2 + TUTORIAL_DESC_1, Email.MESSAGE_CONSTRAINTS);

        // invalid address
        assertParseFailure(parser, NAME_DESC_BOB + PHONE_DESC_BOB + EMAIL_DESC_BOB + INVALID_HANDLE_DESC
                        + TUTORIAL_DESC_2 + TUTORIAL_DESC_1, TelegramHandle.MESSAGE_CONSTRAINTS);

        // invalid tag
        assertParseFailure(parser, NAME_DESC_BOB + PHONE_DESC_BOB + EMAIL_DESC_BOB + HANDLE_DESC_BOB
                        + INVALID_TUTORIAL_DESC + VALID_TUTORIAL_1, "Tutorial has invalid name.");

        // two invalid values, only first invalid value reported
        assertParseFailure(parser, INVALID_NAME_DESC + PHONE_DESC_BOB + EMAIL_DESC_BOB + INVALID_HANDLE_DESC,
                        Name.MESSAGE_CONSTRAINTS);

        // non-empty preamble
        assertParseFailure(parser,
                        PREAMBLE_NON_EMPTY + NAME_DESC_BOB + PHONE_DESC_BOB + EMAIL_DESC_BOB + HANDLE_DESC_BOB
